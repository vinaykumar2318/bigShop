  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const rawData = data.products.filter(product => product.category === 'Hoodie');
        let hoodies = {};
        for (let item of rawData) {
          if (item.title in hoodies) {
            if (!hoodies[item.title].color.includes(item.color.toLowerCase())) {
              hoodies[item.title].color.push(item.color.toLowerCase());
            }
            ...
            hoodies[item.title].color = [item.color.toLowerCase()]
  ...
  return (
    <div className='mt-36 md:mt-14'>
      <section className="text-gray-600 body-font">
        <h1 className="text-4xl font-bold text-center text-gray-800 pt-10">Hoodies</h1>
