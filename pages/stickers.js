  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const rawData = data.products.filter(product => product.category === 'Sticker');
        let stickers = {};
        for (let item of rawData){
          if(item.title in stickers){
            if (!stickers[item.title].color.includes(item.color.toLowerCase())) {
              stickers[item.title].color.push(item.color.toLowerCase());
            }              
            ...
            stickers[item.title].color = [item.color.toLowerCase()]
  ...
  return (
    <div>
      <section className="text-gray-600 body-font">
        <h1 className="text-4xl font-bold text-center text-gray-800 mt-20">Stickers</h1>
